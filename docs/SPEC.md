...
Editors ──► Pages CMS (app.pagescms.org, email-code login)
                 ...
                 ▼
GitHub: Aitkenvale/LSA-website (private, source of truth)
                 │ push triggers build
                 ▼
...
**townsville.bahai.org.au** (NSA-controlled zone): NSA adds
`CNAME townsville.bahai.org.au → sites.bahaitownsville.org.au`. Our side is
ready: Cloudflare for SaaS custom hostname created (cert auto-issues on first
traffic) + Worker route `townsville.bahai.org.au/*` → lsa-website. Once live,
this becomes the canonical public address; bahaitownsville.org.au will 301 to it
...
| `VENUE_CALENDAR_ID` | `f29692572fdc99e569105d75f4bb68aa4b68bcea5b7bf5b324253fdc7fd90e3c@group.calendar.google.com` |
...
sidebar order: Home · What We Believe · What We Do · Community Centre · Pages ·
Centre photo gallery · Events · News · Site settings.
...
| What We Believe / What We Do | `src/content/pages/*.md` | title, intro, hero image, sections[] (heading, text, optional image+alt). Images always render on the RIGHT of text (no alternating) |
| Community Centre | `src/content/centre/centre.md` | page fields **plus** hire settings: hireHeading, hireCalendarNote, hireIntro, hireRates[] |
| Pages (future) | `src/content/extra-pages/*.md` | auto-routed by `src/pages/[slug].astro` at `/<file-name>/`; not in nav |
| Gallery | `src/content/gallery/*.yml` | image, caption, order |
...
| Site settings | `src/content/settings/site.yml` | siteName, tagline (SEO description), contactEmail, bookingEmail (hire recipient — CMS-editable), phone, address, facebook, footer{text, acknowledgement}, sitePassword (pre-launch gate; clear to launch) |

Media: CMS uploads → `src/assets/uploads/`, referenced relatively
(`../../assets/uploads/x.jpg`); Astro emits responsive WebP at build.
...
- **/api/hire** (`src/pages/api/hire.ts`): honeypot (`website` field → fake
  success) → field validation (incl. two required checkboxes: public liability
  insurance, alcohol & drug free; endTime > startTime) → Turnstile siteverify →
...
9. Daily rebuild cron (see §10 — pending here too).
...
- NSA CNAME not yet added; on it going live: make townsville.bahai.org.au
  ...
  NSA mailbox (currently interim lsatownsville@gmail.com).
...
- Pages CMS caches parsed config; after editing `.pages.yml` outside the app,
  force re-read via Admin → Configuration → tiny edit → Save.
...
Nine-pointed star mark (`Logo.astro`, favicon). Sections open with a small gold
